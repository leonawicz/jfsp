...
#' These include \code{breaks}, a vector of breaks applicable to time series plots with years along the x-axis, and
...
#'
#' @examples
#' jfsp_plot(burnArea, 1950:2013, "ba_box", log = TRUE)
jfsp_plot <- function(x, years = NULL, type = NULL, by_rcp = TRUE, col = NULL){
  o <- list(...)
  if(is.null(o$family)) family <- "sans"
  ...
  thm <- .thm(base_size = 20, base_family = family)
  ...
  if(type == "ba_sd"){
    if(is_hist) p <- ggplot2::ggplot(x, aes_string("Year", "BA_sd_ma10", color = clr_var,
    ...
    p <- p + ggplot2::geom_line(size = 1) +
      ...
      scm + sfm + thm + .thm_adj("topright") + gde +
    ...
    if(is_hist) p <- ggplot2::ggplot(x, aes_string("Year", "CBA", colour = clr_var, group = grp_var))
    if(!is_hist) p <- ggplot2::ggplot(x, aes_string("Year", "CBA", colour = clr_var, linetype = lty_var))
    p <- p + ggplot2::geom_step() + ggplot2::facet_wrap(as.formula("~FMO"), scales = "free_y") +
      scm + slm + thm + .thm_adj("topleft") + gde +
    ...
    p <- ggplot2::ggplot(x, aes_string("RCP", y_var, colour = clr_var)) +
      ggplot2::geom_boxplot(outlier.shape = NA) +
      ggplot2::geom_point(aes_string(fill = clr_var), pch = 21, alpha = 0.5,
                 position = position_jitterdodge(jitter.width = 0.5, dodge.width = 0.75)) +
      ggplot2::facet_wrap(as.formula("~FMO"), scales = "free_y") +
      sfm + scm + thm + .thm_adj("topright") +
    ...
    if(is_hist) p <- ggplot2::ggplot(x, aes_string("Year", "value", linetype = "cost")) +
      ggplot2::geom_line() + ggplot2::geom_point()
    if(!is_hist)
      p <- ggplot2::ggplot(x, aes_string("Year", "value", colour = clr_var, linetype = lty_var)) +
      ggplot2::geom_line() + ggplot2::geom_point(shape = 21) + ggplot2::facet_wrap(as.formula("~cost"))
    p <- p + thm + sfm + scm + slm + .thm_adj("topleft") + gde +
      ggplot2::scale_x_continuous(limits = range(years), breaks = breaks) +
    ...
    p <- ggplot2::ggplot(x, aes_string("factor(Decade)", "Mean", color = clr_var,
                                       fill = clr_var, linetype = lty_var)) +
      ggplot2::geom_errorbar(aes_string(ymin = "`5th percentile`", ymax = "`95th percentile`"),
      ...
      sfm + scm + slm + thm + .thm_adj("topright") + gde +
    ...
    p <- ggplot2::ggplot(x, aes_string("Year", "Val", colour = "RCP", linetype = "Tx")) +
      ggplot2::geom_line(size = 1) +
      slm + scm + thm + .thm_adj("topright", "vertical") + gde +
      ggplot2::scale_x_continuous(limits = range(years), expand = c(0, 0), breaks = breaks) +
      ggplot2::labs(title = paste(min(years), "-", max(years), "coniferous:deciduous ratio"),
                    subtitle = subtitle, y = "Ratio")
  } else if(type == "pfire"){
    p <- ggplot2::ggplot(x, aes_string("buffer", "value", colour = "RCP", linetype = "Tx")) +
      ggplot2::geom_line(size = 1) +
...
.thm_adj <- function(position, direction = "horizontal"){
  pos <- switch(position, topright = c(1, 1), topleft = c(0, 1))
  just <- switch(position, topright = "right", topleft = "left")
  ggplot2::theme(plot.margin = ggplot2::unit(c(0.5, 1.5, 0.5, 0.5), "lines"), text = ggplot2::element_text(size = 60),
